    ...
    case 'GET':
      try {
        ...
        if (shopId) {
          ...
          const shop = await fileStore.getShop(shopId)
          const threads = await fileStore.getThreads()
          for (let thead of threads) {
            if (thead && thead.threadId?.startsWith('thread_')) {
              const threadData = await openai.beta.threads.messages.list(thead.threadId, {
                order: 'desc',
                limit: 20
              })
              const items = threadData.data.map(v => ({
                role: v.role,
                type: 'message',
                content: v?.content[0]?.text?.value || " "
              }))
              if (items.length) {
                const conversation = await openai.conversations.create({
                  items: items
                })
                if (conversation) await fileStore.saveThread([{ ...thead, threadId: conversation.id }])
              }
            }
          }
          const assistant = await openai.beta.assistants.retrieve(shop.assistantId)
          console.log('assistant', assistant)
          const updatedShop = await Shop.findOneAndUpdate(
            { _id: shopId },
            { $set: {instructions: assistant.instructions, model: assistant.model} },
