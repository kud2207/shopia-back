import Motif from "src/@apiCore/models/motif";
  ...
  switch (method) {
    case "GET":
      try {
        ...
        const zones = await Motif.aggregate(data);
        res.status(200).json({
          success: true,
          data: !req.query.page
            ? zones
            : zones.length > 0
              ? zones[0].data
              : [],
          total: !req.query.page
            ? zones.length
            : zones.length > 0
              ? zones[0].pagination[0]?.total || 0
              : 0,
        });
      } catch (error) {
        console.log(error);
    ...
    case "POST":
      try {
        ...
        form.parse(req, async (err, fields, files) => {
          ...
          const motif = await Motif.create(fields);
          if (motif) res.status(201).json({ success: true, data: motif });
