...
export default async function register(req, res) {
  ...
  switch (method) {
    case 'GET':
      try {
        const { page, limit, city, cities } = req.query

        const pageNumber = parseInt(page) || 1
        const pageSize = parseInt(limit) || 5
        let queryData = {}
        if (city) queryData['city._id'] = new mongoose.Types.ObjectId(city)
        if (cities) {
          queryData['city._id'] = { $in: cities?.split(',')?.map(city => new mongoose.Types.ObjectId(city)) }
        }
        ...
        const zones = await Zone.aggregate(data)
        res.status(200).json({
          success: true,
    ...
    case 'POST':
      try {
        ...
        form.parse(req, async (err, fields, files) => {
          ...
          const zone = await Zone.create(fields)
          if (zone) {
