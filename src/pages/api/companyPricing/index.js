...
export default async function register(req, res) {
  ...
  switch (method) {
    case 'GET':
      try {
        const { page, limit, type, company } = req.query

        const pageNumber = parseInt(page) || 1
        const pageSize = parseInt(limit) || 5
        let queryData = {}
        if (type) queryData.type = req.query.type
        if (company) queryData.deliveryCompany = new mongoose.Types.ObjectId(company)
        ...
        const zones = await DeliveryPricing.aggregate(data)
        res.status(200).json({
          success: true,
    ...
    case 'POST':
      try {
        ...
        form.parse(req, async (err, fields, files) => {
          ...
          const deliveryPricing = await DeliveryPricing.create(fields)
          if (deliveryPricing) {
            if (deliveryPricing.deliveryCompany) {
              const company = await DeliveryCompany.findOne({ _id: deliveryPricing.deliveryCompany })
              await company.addPricingItem(deliveryPricing._id)
            }
            res.status(201).json({ success: true, data: deliveryPricing })
