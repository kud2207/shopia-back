...
import { createUser, getUsersById, updateUser } from 'src/@apiCore/npoints'
...
export async function getStaticProps({ locale }: any) {
  return {
    props: {
      ...(await serverSideTranslations(locale ?? 'fr', ['common', 'account', 'reset']))
    }
  }
}
...
const AccountSettings = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    email: user?.email,
    role: user?.role,
    imgSrc: user?.image || '/images/avatars/1.png',
    phone: user?.phone,
    gender: user?.sexe,
  ...
  useEffect(() => {
    if (activeShop) setSelectedShops([activeShop])
  }, [activeShop])
  useEffect(() => {
    if (id) {
      setLoad1(true)
        ...
        if (response.status == 200) {
          ...
          if (data) {
            setFormData({
              ...
              logoSrc: data?.logo || '/images/logos/service.png',
  ...
  const submitFormData = () => {
    ...
    formValues.append('pass', formData.pass)
    formValues.append('shops', selectedShops.map((shop: any) => shop._id)?.toString())
    if (id) {
          ...
          {!load1 && (
            <form onSubmit={handleSubmit}>
              <Grid container spacing={7}>
                ...
                <Grid item xs={12} sm={6}>
                  <FormControl fullWidth>
                    ...
                    <Select
                      ...
                      required
                    >
                      <MenuItem selected={formData.role == 'shop_admin'} value={'shop_admin'}>
                        {t('shop_admin')}
                      </MenuItem>
                      <MenuItem selected={formData.role == 'gestionnaire'} value={'gestionnaire'}>
                        {t('gestionnaire')}
                      </MenuItem>
                      <MenuItem selected={formData.role == 'caissiere'} value={'caissiere'}>
                        {t('caissiere')}
                      </MenuItem>
                    </Select>
                  </FormControl>
                ...
                <Grid item xs={12}>
                  ...
                  {isSuccess && (
                    <Alert severity='success' sx={{ mb: 4 }}>
                      {t(id ? 'success_update' : 'success_create')}
                    </Alert>
                  )}
                </Grid>
                ...
                <Grid item xs={12}>
                  <Button
                    disabled={formData.pass && !isValid ? true : false}
                    variant='contained'
                    sx={{ marginRight: 3.5 }}
                    type='submit'
                  >
                    {t(id ? 'save' : 'add')} {load && <CircularProgress size={20} sx={{ ml: 5 }} color='inherit' />}
                  </Button>
                </Grid>
              </Grid>
            </form>
