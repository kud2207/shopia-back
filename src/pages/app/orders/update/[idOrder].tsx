...
import { getOrderById, getOrderUsers, getProducts, updateOrder } from 'src/@apiCore/npoints'
    ...
    getOrderById(id)
      .then(resp => {
        if (resp.status == 200) {
          ...
          setCustomerId(respData.customer._id)
          const formattedItems = respData.items.map((item: any) => ({
          ...
          setOrderDate(moment(respData.date).toISOString().split('T')[0])
          setDevAddress(respData?.deliveryInfo?.address||"")
          setDevDate(respData?.deliveryInfo?.date)
        }
      })
      .catch(err => {
        console.log(err)
    ...
    updateOrder(`${idOrder}`, data)
      .then(response => {
        if (response.status == 200) {
          ...
          router.push('/app/orders/' + idOrder)
  ...
  return (
    <CardContent sx={{p:0}}>
      <form onSubmit={handleSubmit}>
        <Box>
          <Typography variant='h4'>{t('update_title')}</Typography>
        ...
        <Grid container spacing={7} mt={4}>
          ...
          <Grid item xs={12}>
            <Button variant='contained' disabled={load} type='submit' sx={{ marginRight: 3.5 }}>
              {t('update_btn')} {load && <CircularProgress size={20} sx={{ ml: 5 }} color='inherit' />}
            </Button>
          </Grid>
        </Grid>
      </form>
